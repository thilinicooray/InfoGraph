...
class Encoder(torch.nn.Module):
    ...
    def forward(self, data, psuedo):
        ...
        input = torch.cat([data.x, psuedo.unsqueeze(1)], -1)

        out = F.relu(self.lin0(F.normalize(input, p=2, dim=-1)))
...
class Net(torch.nn.Module):
    ...
    def supervised_loss(self, data, psuedo_label):
        ...
        reconstruction_error = 0.01* self.recon_loss1(reconstructed_node, data.edge_index, data.batch)
