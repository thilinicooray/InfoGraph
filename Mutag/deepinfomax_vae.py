...
class GcnInfomax(nn.Module):
    ...
    def forward(self, x, edge_index, batch, num_graphs):
        ...
        loss = reconstruction_error + class_kl_divergence_loss + node_kl_divergence_loss
    ...
    #for seed in [42]:
        for batch_size in batch:
            for hidden_size in hidden_sizes:
                for num_layer_count in layers:
                    for node_ratio in node_dims:

                        node_dim = int(hidden_size*2*node_ratio)

                        class_dim  = int(hidden_size*2 - node_dim)
                        ...
                        print('current setup -> seed batch size hidden layers, class dim, node_dim', seed, batch_size, hidden_size,
                              num_layer_count, node_dim, class_dim)
                        ...
                        args.num_gc_layers = num_layer_count
                        args.hidden_dim = hidden_size
