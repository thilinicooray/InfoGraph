...
if __name__ == '__main__':
    ...
    for seed in [52]:
        ...
        optim_P_enc = torch.optim.Adam(model.decoder.parameters(), lr=gen_lr)
        optim_Q = torch.optim.Adam(model.encoder.parameters(), lr=gen_lr)
        ...
        for epoch in range(1, epochs+1):
            ...
            for data in dataloader:
                ...
                reconstructed_node = model.decoder(node_latent_embeddings, class_latent_embeddings)
                reconstruction_error = model.recon_loss1(reconstructed_node, data.edge_index, data.batch)

                recon_loss_all += reconstruction_error.item()
