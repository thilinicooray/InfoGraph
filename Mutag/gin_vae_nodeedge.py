...
class Encoder(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers):
        ...
        for i in range(num_gc_layers+4+4):
        ...
        # node
        self.edge_mu = Linear(in_features=dim, out_features=dim, bias=True)
        self.edge_logvar = Linear(in_features=dim, out_features=dim, bias=True)

        # class
        self.eclass_mu = Linear(in_features=dim, out_features=dim, bias=True)
        self.eclass_logvar = Linear(in_features=dim, out_features=dim, bias=True)
    ...
    def forward(self, x, edge_index, batch):
        ...
        edge_rep = x[edge_index[0]] * x[edge_index[1]]

        edge_latent_mu = self.bns[j+4](F.relu(self.edge_mu(edge_rep)))
        edge_latent_logvar = self.bns[j+5](F.relu(self.edge_logvar(edge_rep)))

        eclass_mu = self.bns[j+6](F.relu(self.eclass_mu(edge_rep)))
        eclass_logvar = self.bns[j+7](F.relu(self.eclass_logvar(edge_rep)))
        ...
        return node_latent_space_mu,  node_latent_space_logvar, class_latent_space_mu, class_latent_space_logvar, edge_latent_mu, edge_latent_logvar, eclass_mu,eclass_logvar
