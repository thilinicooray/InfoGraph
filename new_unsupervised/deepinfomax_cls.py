...
class GcnInfomax(nn.Module):
    def __init__(self, hidden_dim, num_gc_layers, alpha=0.5, beta=1., gamma=.1):
        ...
        self.classifier = LogReg(hidden_dim, 128)
    ...
    def forward(self, x, edge_index, batch, num_graphs):
        ...
        cls_loss = self.classifier(class_latent_embeddings, batch)
        ...
        loss =  class_kl_divergence_loss + node_kl_divergence_loss + 1e-7*reconstruction_error + cls_loss

        loss.backward()


        return  1e-7*reconstruction_error.item(), class_kl_divergence_loss.item() , node_kl_divergence_loss.item(), cls_loss.item()
    ...
    for seed in [32,42,52,62,72]:
        ...
        for epoch in range(1, epochs+1):
            ...
            for data in dataloader:
                ...
                optimizer.zero_grad()
                recon_loss, kl_class, kl_node, cls_loss = model(data.x, data.edge_index, data.batch, data.num_graphs)
                recon_loss_all += recon_loss
                kl_class_loss_all += kl_class
                kl_node_loss_all += kl_node
                cls_loss_all += cls_loss_all
            ...
            print('Epoch {}, Recon Loss {} KL class Loss {} KL node Loss {} Cls Loss {}'.format(epoch, recon_loss_all / len(dataloader),
                                                                                    kl_class_loss_all / len(dataloader), kl_node_loss_all / len(dataloader),
                                                                                                cls_loss_all / len(dataloader)))
            #print('\n\n', losses, '\n')
