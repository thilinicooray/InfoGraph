...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        node_latent_space_mu = self.bns[j](torch.sigmoid(self.convs[j](x, edge_index)))
        node_latent_space_logvar = self.bns[j+1](torch.sigmoid(self.convs[j+1](x, edge_index)))

        class_latent_space_mu = self.bns[j+2](torch.sigmoid(self.convs[j+2](x, edge_index)))
        class_latent_space_logvar = self.bns[j+3](torch.sigmoid(self.convs[j+3](x, edge_index)))
