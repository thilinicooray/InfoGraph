...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        if self.training:
            ...
            return node_latent_space_mu,  node_latent_space_logvar, class_latent_space_mu, class_latent_space_logvar,  xs[-1]
...
class Decoder(torch.nn.Module):
    def __init__(self, node_dim, class_dim, feat_size):
        ...
        self.linear_model = torch.nn.Sequential(OrderedDict([
            ('linear_1', torch.nn.Linear(in_features=node_dim*2, out_features=feat_size, bias=True)),
            ('relu_1', Tanh()),
