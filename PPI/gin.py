...
class Decoder(torch.nn.Module):
    def __init__(self, node_dim, class_dim, feat_size):
        ...
        self.linear_model = torch.nn.Sequential(OrderedDict([
            ('linear_1', torch.nn.Linear(in_features=node_dim*3, out_features=feat_size, bias=True)),
            ('relu_1', Tanh()),
    ...
    def forward(self, node_latent_space, class_latent_space, combo):
        x = torch.cat((node_latent_space, class_latent_space, combo), dim=1)
