...
class GcnInfomax(nn.Module):
    ...
    def forward(self, x, edge_index, batch, num_graphs):
        ...
        #node sum for group
        ngrouped_mu, ngrouped_logvar = accumulate_group_evidence(
            node_mu.data, node_logvar.data, batch, True
        )
        ngroup_latent_embeddings = group_wise_reparameterize(
            training=True, mu=ngrouped_mu, logvar=ngrouped_logvar, labels_batch=batch, cuda=True
        )
    ...
    def get_embeddings(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                ret.append((self.node_discriminator(node_latent_embeddings_only)).cpu().numpy())
