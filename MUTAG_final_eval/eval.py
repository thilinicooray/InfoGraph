    ...
    def get_embeddings(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                sim_node = torch.tanh(torch.matmul(entangled[0].unsqueeze(-1).t(), node_mu[0].unsqueeze(0))).cpu().numpy()
                graph_node = torch.tanh(torch.matmul(entangled[0].unsqueeze(-1).t(), class_mu[0].unsqueeze(0))).cpu().numpy()
