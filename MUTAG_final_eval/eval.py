    ...
    def get_embeddings(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                sim_node = torch.sigmoid(torch.matmul(entangled[0].unsqueeze(0).t(), accumulated_class_latent_embeddings[0].unsqueeze(0))).cpu().numpy()
                graph_node = torch.sigmoid(torch.matmul(entangled[1].unsqueeze(0).t(), accumulated_class_latent_embeddings[1].unsqueeze(0))).cpu().numpy()
