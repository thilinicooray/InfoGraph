    ...
    def get_embeddings(self, loader):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                sim_node = torch.sigmoid(torch.matmul(entangled[0].unsqueeze(0).t(), class_mu[0].unsqueeze(0))).cpu().numpy()
                graph_node = torch.sigmoid(torch.matmul(entangled[1].unsqueeze(0).t(), class_mu[1].unsqueeze(0))).cpu().numpy()
