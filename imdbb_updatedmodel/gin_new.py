...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch):
        ...
        glob_prop = self.bns[j](F.relu(self.convs[j](global_weights*x, edge_index)))
        local_prop = self.bns[j+1](F.relu(self.convs[j+1]((1-global_weights)*x, edge_index)))

        global_n = global_add_pool(glob_prop, batch)


        node_latent_space_mu = self.bns[j+2](torch.tanh(self.node_mu(local_prop)))
        node_latent_space_logvar = self.bns[j+3](torch.tanh(self.node_logv(local_prop)))

        class_latent_space_mu = self.bns[j+4](torch.tanh(self.cls_mu(global_n)))
        class_latent_space_logvar = self.bns[j+5](torch.tanh(self.cls_logv(global_n)))
