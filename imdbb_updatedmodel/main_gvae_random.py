        ...
        with torch.no_grad():
            for data in loader:
                ...
                if not dataset.num_features:
                    ...
                    y = torch.LongTensor(x.size(0),1).random_() % nb_digits
                    # One hot encoding buffer that you create out of the loop and just keep reusing
                    y_onehot = torch.FloatTensor(x.size(0), nb_digits)
                    y_onehot.zero_()
                ...
                for epoch in range(1, epochs+1):
                    ...
                    for data in dataloader:
                        ...
                        if not dataset.num_features:
                            ...
                            nb_digits = 20
                            # Dummy input that HAS to be 2D for the scatter (you can use view(-1,1) if needed)
                            y = torch.LongTensor(data.x.size(0),1).random_() % nb_digits
                            # One hot encoding buffer that you create out of the loop and just keep reusing
                            y_onehot = torch.FloatTensor(data.x.size(0), nb_digits)
                            y_onehot.zero_()
                            y_onehot.scatter_(1, y, 1)
                            data.x = y_onehot
