...
def train(epoch, use_unsup_loss):
    ...
    if use_unsup_loss:
        for data, data2 in zip(train_loader, unsup_train_loader):
            ...
            sup_loss = F.mse_loss(pred.squeeze(), data.y)



            unsup_loss = model.info_loss(data2)
        ...
        for data in train_loader:
            ...
            pred, contras_loss = model.info_loss(data)
...
if __name__ == '__main__':
    seed_everything()
    from model_infograph_semi import Net
    ...
    model = Net(dataset.num_features, dim, 1, use_unsup_loss, separate_encoder).to(device)
