...
class Net(torch.nn.Module):
    def __init__(self, num_features, dim, target, use_unsup_loss=False, separate_encoder=False):
        ...
        self.sup_encoder = Encoder(num_features, dim)
    ...
    def our_loss(self, data):
        ...
        node_logvar = F.relu(self.node_lv_conv(out, data.edge_index, data.edge_attr))
        ...
        g_mu = self.set2set_mu(graph_mu_id, data.batch)
        g_lv = self.set2set_lv(graph_lv_id, data.batch)

        _, count = torch.unique(data.batch,  return_counts=True)

        grouped_mu = torch.repeat_interleave(g_mu, count, dim=0)
        grouped_logvar = torch.repeat_interleave(g_lv, count, dim=0)
    ...
    def unsup_sup_loss(self, data):
        ...
        graph_mu_id_s = F.relu(self.graph_mu_conv(sup_out, data.edge_index, data.edge_attr))
        graph_lv_id_s = F.relu(self.graph_lv_conv(sup_out, data.edge_index, data.edge_attr))
