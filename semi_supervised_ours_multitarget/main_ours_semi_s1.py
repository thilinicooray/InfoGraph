...
if __name__ == '__main__':
    seed_everything()
    from model_ours_semi import Net
...
def train(epoch, use_unsup_loss):
    ...
    if use_unsup_loss:
        for data, data2 in zip(train_loader, unsup_train_loader):
            ...
            combo_pred = model.forward_joint(data)

            sup_loss_combo = (F.mse_loss(combo_pred[:,0], data.y[:,0]) + F.mse_loss(combo_pred[:,1], data.y[:,1]) + F.mse_loss(combo_pred[:,2], data.y[:,2])
                        + F.mse_loss(combo_pred[:,3], data.y[:,3])  + F.mse_loss(combo_pred[:,4], data.y[:,4]) + F.mse_loss(combo_pred[:,5], data.y[:,5])
                        + F.mse_loss(combo_pred[:,6], data.y[:,6]) + F.mse_loss(combo_pred[:,7], data.y[:,7]) + F.mse_loss(combo_pred[:,8], data.y[:,8])
                        + F.mse_loss(combo_pred[:,9], data.y[:,9]) + F.mse_loss(combo_pred[:,10], data.y[:,10]) + F.mse_loss(combo_pred[:,11], data.y[:,11]))/12
            ...
            loss = 20*sup_loss + unsup_loss + unsup_sup_loss * lamda + 20*sup_loss_combo
...
def test(loader, std_all):
    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            error += torch.sum((model.forward_joint(data) * stds_allbatch - data.y * stds_allbatch).abs(),0)  # MAE
...
if __name__ == '__main__':
    seed_everything()
    from model_ours_semi_s1 import Net
    from arguments import arg_parse
    args = arg_parse()
